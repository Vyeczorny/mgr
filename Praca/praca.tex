...
Swift to język programowania stworzony przez firmę Apple głównie na potrzeby tworzenia aplikacji na platformy iOS, Mac OS, tvOS i WatchOS. Jego pierwsza wersja została ogłoszona 2 czerwca 2014 roku podczas konferecji WWDC. 

Przed 2014 rokiem głównym językiem używanym do tworzenia oprogramowania na systemy operacyjne Apple był język Objective-C. Powstał on w 1983 roku jako nakładka na C rozszerzona o elementy obiektowe wzrorowane na Smalltalku. Głównym problemem Objective-C jest wysoki poziom trudności nauki. Składnia tego języka różni sie znacznie od składni innych popularnych języków obiektowych, jest dużo bardziej rozwlekła i mniej czytelna. Również wydajność nie jest najlepsza, w celu uzyskania szybko działającego kodu programiści często wykorzystują fakt, że Objective-C współpracuje z C i piszą najważniejsze części aplikacji właśnie w tym języku. Swift ma za zadanie rozwiązać właśnie te problemy. Jego składnia jest dużo prostsza i bardziej podobna do innych obiektowych języków programowania. Zaadresowano również zwiększenie wydajności i bezpieczeństwa kodu. Swift otrzymał nowe funkcje pozwalające pisać kod szybciej, wydajniej i bezpieczniej. Dokładne porównanie obydwu języków znajduje się w rozdziale \ref{ch:swift-objc}.

Swift zaczął bardzo szybko zyskiwać na popularności. Wg ankiety przeprowadzanej co roku przez serwis \url{StackOverflow.com} już w 2017 roku nowy język był bardziej popularny od swojego poprzednika \footnote{https://insights.stackoverflow.com/survey/2017\#most-popular-technologies}. Należy również zwrócić uwagę na kategorię "Most Loved Languages", w której Swift rokrocznie zajmuje miejsce w pierwszej piątce.

3 grudnia 2015 roku kod źródłowy języka został udostępniony na serwisie GitHub. W tym samym momencie ogłoszono również wsparcie dla systemów z rodziny Linux. Te dwa wydarzenie pozwoliły na myślenie o szerszym użyciu tego języka, niż tylko aplikacje mobilne. W niedługim czasie powstało kilka ram do tworzenia aplikacji serwerowych, takich jak Kitura, Vapor czy Perfect. Firma IBM dodała również wsparcie dla Swifta w swoich serwisach chmurowych. Kolejnym potencjalnym zastosowaniem jest uczenie maszynowe. Pierwszą próbą w tym kierunku jest wersja platformy TensorFlow wykorzystująca Swift.

Pomimo szybko rosnącej popularności Swifta, istnieje mało źródeł traktujących o wydajności nowego języka. W Internecie można znaleźć kilka stron i blogów zawierających testy wydajnościowe, takich jak np. "The Computer Language Benchmark Game \footnote{\url{https://benchmarksgame-team.pages.debian.net/benchmarksgame/faster/swift.html}}", żaden z autorów nie pokusił się jednak o bardziej wnikliwą analizę. 
...
%      &  & Swift & Objective-C &  \\
%      \hline
%     \texttt{ArrayInsertion}         & 2500000 & 11,63 $ms$ & 55,68 $ms$ & -78,32 \% \\ 
%     \texttt{Fibonacci}              & 30 & 4,70 $ms$ & 9,41 $ms$ & -45,84 \% \\ 
%     \texttt{BubbleSort}             & 7500 & 60,59 $ms$ & 481,06 $ms$ & -87,69 \% \\ 
%     \texttt{BinarySearchTree \newline Classis } & 50000 & 188,21 $ms$ & 82,75 $ms$ & +129,29 \% \\ 
%     \texttt{BinarySearchTree \newline Enums } & 50000 & 169,67 $ms$ & 82,75 $ms$ & + 103,20 \% \\ 
%     \texttt{BinarySearchTree \newline Optimized} & 50000 & 20,49 $ms$ & 74,16 $ms$ & -72.09 \% \\ 
%     \texttt{DispatchMethod \newline (Static vs Message)} & 5000000 & 5,96 $ms$ & 24,69 $ms$ & -75,00 \% \\ 
%     \texttt{DispatchMethod \newline (Dynamic vs Message)} & 5000000 & 54,00 $ms$ & 24,69 $ms$ & +112,79 \% \\ 
%     \texttt{Dijkstra}               & 1500      & 115,14 $ms$   & 392,48 $ms$   & -65,33 \%     \\ 
%     \texttt{DijkstraOptimized}      & 1500      & 61,28 $ms$    & 52,47 $ms$    & -5,14 \%      \\ 
%     \texttt{WordFrequency}          & 25000     & 21,88 $ms$    & 8,67 $ms$     & +157,48 \%    \\ 
%     \texttt{SieveOfEratosthenes}    & 25000     & 36,35 $ms$    & 20,71 $ms$    & +70,94 \%     \\ 
%     \texttt{CountLinesWordsChars}   & 50000     & 2,27 $ms$     & 0.03 $ms$     & +7718,38 \%   \\ 
%     \texttt{StringConcatenation}    & 200000    & 8,33 $ms$     & 11,78 $ms$    & -22,48 \%     \\
%     \texttt{RGBHistogram}           & 50000     & 1,40 $ms$     & 14,42 $ms$    & -90,21 \%     \\ 
